...
#include <allegro5/allegro_font.h> // for displaying the player's score, and showing a humiliating 'game over'
#include <allegro5/allegro_primitives.h> // draw some stars
#include <allegro5/allegro_audio.h> // pew pew sounds
#include <allegro5/allegro_acodec.h> // pew pew sounds
...
// generate random numbers within a range (lo and hi)
...
// pulling the individual sprites into their separate bitmaps
...
// function to destroy all of the bitmaps when the program ends
void sprites_deinit() {
 ...
 *
 * We're looping through the fx array until we find one that isn't used. 
 * Once we do, we set it up with the given coordinates, and whether it's to be a spark or not
 * In the case of explosions, we also take the opportunity to play the audios
 * 
 */
...
/*
 * Run every frame, and makes sure that the frame variables of used effects are incremented. 
 * Once the effect has run its course, it is returned to being un-used.
 */
void fx_update() {
    for(int i = 0; i < FX_N; i++)
    {
...
// loops through the fx array yet again - but this time, with the intent of drawing used effects to the screen.
void fx_draw() {
    for(int i = 0; i < FX_N; i++)
    {
        if(!fx[i].used)
...
typedef struct SHOT {
    int x, y, dx, dy;
    int frame;
    bool ship;  // if ship is true, the player's ship fired this shot. Otherwise, it belonged to an alien
    bool used;
} SHOT;
...
// called when the player or an alien wants to shoot
...
void ship_update() {
    ...
    // we don't want them flying out of the playfield
    if(ship.x < 0)
        ship.x = 0;
    if(ship.y < 0)
        ship.y = 0;
...
void ship_draw() {
    ...
    if(ship.respawn_timer)
        return;
    if(((ship.invincible_timer / 2) % 3) == 1) // don't show the ship on the 2nd and 3rd frame in every 6 frames
        return;
...
typedef struct ALIEN
{
    ...
    int shot_timer;
    int blink;
    int life; // the number of hits the alien can take before it explodes
    bool used;
} ALIEN;
...
typedef struct STAR {
    float y;
    float speed;
    // there's no used this time, because all of the stars are permanently displayed
} STAR;
...
// scatter the stars across the Y-axis randomly
void stars_init() {
...
// adds speed to y on every frame
void stars_update() {
    for(int i = 0; i < STARS_N; i++)
    {
